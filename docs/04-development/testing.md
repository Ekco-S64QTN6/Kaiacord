...
```
Only **4** tests need Ollama or a GPU. The rest of what that invocation deselects is the 82
marked `slow`. A bare `pytest -q` runs them, which loads `gemma3:12b` and evicts the production
