...
class UnifiedLogger:
    ...
    def _update_stats_from_log(self, log_entry):
        ...
        from utils.stats_tracker import stats_tracker  # Import here to avoid circular dependency
...
def replace_all_logging():
    ...
    # Store original print if not already stored
    ...
    def unified_print(*args, **kwargs):
        if args:
            ...
            if "|" in message and len(message.split("|")) >= 2:
                ...
                if len(parts[0].strip().split(":")) == 3:  # Looks like HH:MM:SS
                    if len(parts) > 1 and len(parts[1].strip().split(":")) == 3:
                        # Has two timestamps, remove first
    ...
    # Also intercept stdout
    class UnifiedStdout:
        def write(self, text):
            if text.strip():
                # Avoid infinite recursion if logger prints to stdout
                if not text.startswith('\033'): # Skip color codes
                     logger.log(text.strip(), "INFO")
        
        def flush(self):
            pass
    
    # Only replace stdout if we are sure it won't cause issues
