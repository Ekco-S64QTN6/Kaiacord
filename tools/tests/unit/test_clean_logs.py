...
# Let's target the exact words we saw:
# it's, is, that's, my, this, more, human,
# Actually, Kaia just sprinkles it everywhere.
# We want to replace "\b([^."\n\r]+?)[\u2026\.]{2,3}\s+" -> " " ? No, we want to keep the word before.
# Let's try matching any letter followed by ellipsis and space.
clean_text = re.sub(r'([a-zA-Z])[\u2026\.]{2,3}\s+([a-zA-Z])', r'\1 \2', text)
# Wait, "this… this" needs to be handled too.
# Let's see what the regex does.
print("=== BEFORE ===")
# print(text)
print("=== AFTER ===")
print(clean_text)
