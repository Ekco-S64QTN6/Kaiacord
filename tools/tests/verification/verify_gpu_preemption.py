...
async def verify_preemption():
    ...
    # 1. Start a low priority task that "takes" 10GB
    log_info("Starting Low Priority Task (EMBEDDING, 10GB)...")
    ...
    # Small sleep to let it start and reserve
    await asyncio.sleep(0.5)
    
    # Verify reservation exists
    if "low_task" in gpu_memory_manager.reservations:
        log_success("Low priority task successfully reserved VRAM.")
    else:
        log_error("Low priority task failed to reserve VRAM.")
        return
        
    # 2. Simulate VRAM being "filled" by the model load
    gpu_memory_manager.get_vram_status = MagicMock(return_value={
        'total': 12.0,
        'allocated': 10.0,
        'reserved': 10.0,
        'free': 2.0 # Only 2GB left
    })
