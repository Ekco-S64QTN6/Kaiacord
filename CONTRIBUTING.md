...
Thanks for your interest in contributing. This guide covers the PR workflow. The runtime
constraints, architecture rules, and subsystem invariants live in
[`CLAUDE.md`](CLAUDE.md) — that is the canonical developer directive, and this file does not
repeat it.

## Before You Start

1. **Read [`CLAUDE.md`](CLAUDE.md).** Especially §2 (running and validating), §3 (registry
   integrity), and §12 (paths not to touch).
...
Python 3.12+, a virtualenv with `requirements.txt` installed, and — for anything that actually
generates text — a running Ollama. You do **not** need a Discord token to validate most changes.
...
Use `venv/bin/python3`, not the system interpreter. `CLAUDE.md` §2 explains what goes wrong
otherwise, and why the failure surfaces hours later somewhere unrelated.
...
# 2026-09-21: 1,660 passed, 10 skipped, 82 deselected, 1 xfailed.
...
If you modified a data registry, run the five integrity checks in `CLAUDE.md` §3 and verify
...
[music] Re-jitter trance arrangement timings
[docs] Update balance review with T5 audit
```

Areas: `ttrpg`, `combat`, `fishing`, `housing`, `alchemy`, `core`, `docs`, `config`, `kaia`,
...
`.env`, `memory/`, `Kaiacord.py`, `knowledge_base/kaia_persona.md`, `config/`, and
`knowledge_base/user_logs/`. `CLAUDE.md` §12 has the reason for each.
...
2. **Wrong indentation** — `droppable_only` at 4-space instead of 8-space indent
   (`CLAUDE.md` §3)
3. **Balance violation** — item stats exceed the tier budget table
4. **Scope creep** — PR does three unrelated things
5. **No validation** — didn't run the suite
